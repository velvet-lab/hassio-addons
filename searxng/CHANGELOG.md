...
- Fix: the Redis connection env var is now named `SEARXNG_VALKEY_URL` (not `SEARXNG_REDIS_URL`). SearXNG maps `${SEARXNG_REDIS_URL}` onto the deprecated `redis.url` setting, which made it treat a disabled Redis as configured and crash with `Valkey URL must specify one of the following schemes ...`. With the env var using the valkey name, `valkey.url` is set correctly and the deprecation path is avoided.
