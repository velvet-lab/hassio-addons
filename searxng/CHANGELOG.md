...
- Fix: pass `--interface wsgi` to Granian. SearXNG's `flaskfix.py` expects a real WSGI `environ` dict (with `.get`). Granian's default RSGI mode hands an `RSGIHTTPScope` object to the app instead, which crashed with `AttributeError: 'RSGIHTTPScope' object has no attribute 'get'`.
