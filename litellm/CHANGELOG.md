...
### Add-on
- Fixed s6-overlay startup: Added `up` file for `litellm-pre` oneshot service
  pointing to `run` (s6-overlay requires `up` for oneshot services)
- Add-on now starts correctly via s6-rc compile
