...
- Disable the Gogs web installer (`INSTALL_LOCK = true`) so it cannot overwrite the add-on-managed `app.ini`, and instead enable **self-registration** (`DISABLE_REGISTRATION = false`): the first user to register automatically becomes the administrator (Gogs' built-in behavior), removing the need for a CLI admin bootstrap.
